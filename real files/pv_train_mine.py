"""This is file trains the neural network on the train dataset"""
...
cwd = os.getcwd()
train_data_filepath = os.path.join(cwd, "..", "data", "train_data.json")
with open(train_data_filepath) as train_file:
...
def train(hidden_size, num_layers):
        ...
        epoch_loss_data = {"total_epochs": total_epochs, "epoch_time": epoch_time, "loss_at_epoch": loss_at_epoch}
        epoch_loss_data_filepath = os.path.join(cwd, "..", "data", "epoch_loss_data.json")
        with open(epoch_loss_data_filepath, "w") as json_file_loader:
            json.dump(epoch_loss_data, json_file_loader) 
        torch.save(f_x.state_dict(),'f_x_base_save_pv.tar')
        torch.save(inn.state_dict(),'inn2_save_pv.tar')
