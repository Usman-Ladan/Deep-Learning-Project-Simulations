...
#defining the dynamics of the lorentz system
device_str='cpu'
def test_fun(t,x):
    sig=10.
    rho=28.
...
tt_tors=torch.tensor([[.15,.15,.15,0.,0.,0.]]) # initial conditions
xy_d_list=[]
for i in range(len(tt_tors)): #for each initial condition. len is lenght along first axis
...
#now xy_d_list is the path of the tragectories, with noise. shape is (len(t_d), 3)
...
def train(hidden_size, num_layers):
        ...
        random.seed(seed)
        np.random.seed(seed)
        #f_x the architecture of the base ODE
        f_x=n.Sequential(
        n.Linear(6, 30),
        n.Tanh(),
        n.Linear(30, 30),
        ...
        def for_inn(x):
            return(inn(x)[0])
        #backward pass through INN, inn is defined lower down
        def rev_inn(x):
        ...
        for i in trange(0, 5000):
            ...
            eval_nl=linear_val_ode2(tt_tors,t_d) # have the estimated times for all t_d
            """
